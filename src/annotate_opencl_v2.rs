...
impl OpenCLv2 {
    ...
    pub fn run_batch(
        ...
        let kernel = self.pq.kernel_builder("ani_lookup_kernel_v2")
            .arg(&self.buf_chrpos)
...
pub fn annotate_vcf_opencl_v2(
    ...
    for (line, cp, rh, ah) in rx {
        ...
        if chrpos.len() >= BATCH {
            flush_batch(
                gpu, ani, &mut bw,
                &mut chrpos, &mut refhash, &mut althash, &mut lines
            )?;
        }
    }
...
fn flush_batch(
    ...
    for i in 0..n {
        ...
        if idx >= 0 {
            ...
            let info = crate::annotate_index::read_cstring(
                &ani.string_block, e.info_ofs as usize);
