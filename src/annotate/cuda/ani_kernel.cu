...
//   - ALPHA_BUCKETS = 0.30, BETA_KEYS = 0.60 → beta_threshold = 39322
//   - fast_reduce: hi-64 of the 128-bit product
//   - h2 derivation: base.rotate_left(23) ^ 0xA24B1F6FDA392B31
//   - slot mix: pilot * 0xA24B1F6FDA392B31, then rotl(31), then * 0xD6E8FEB86659FD93
//   - fingerprint: low 16 bits of canonical hash
//   - bloom: 8-word split-block, ((h>>32) * blocks) >> 32 selects block
//
// **Memory layout pushed to the device per index (allocated once at GpuAni::load):**
//   - prehash_seed (u64), mph_salt (u64), num_buckets (u32), num_slots (u64),
//     prerotate (u8) — uploaded as scalars via cudaMemcpyToSymbol or kernel args.
//   - pilots: u8 array of length `num_buckets`. ~30-50 MB for a 100M-key index.
//   - bloom_words: optional u64 array of length `bloom_blocks * 8`. Power of 2,
//     ~16-20 MB for 100M keys at 11 bits/key.
...
// ---------- 2-zone bucket-for (matches ptrhash25::bucket_for) ------------

// (BETA_KEYS * 65536) = (0.60 * 65536) = 39321.6 → truncated to 39321.
__device__ __forceinline__ unsigned long long ptrhash_bucket_for(unsigned long long h1,
                                                                   unsigned int num_buckets) {
    const unsigned int beta_threshold = 39321u; // BETA_KEYS * 65536
    // ALPHA_BUCKETS = 0.30
    unsigned long long large_buckets = (unsigned long long)((double)num_buckets * 0.30);
    if (large_buckets == 0ULL) large_buckets = 1ULL;
    unsigned long long small_buckets = (unsigned long long)num_buckets - large_buckets;
    if (small_buckets == 0ULL) small_buckets = 1ULL;
...
__device__ __forceinline__ void ptrhash_hash_key(unsigned long long canonical,
    ...
    *h1_out = base;
    *h2_out = rotl64(base, 23u) ^ 0xA24B1F6FDA392B31ULL;
}
...
__device__ __forceinline__ bool bloom_contains(const unsigned long long *bloom_words,
    ...
    #pragma unroll 8
    for (int i = 0; i < 8; ++i) {
        unsigned int bit = (seed * SALT[i] >> 27) & 0x3Fu;
...
__device__ __forceinline__ unsigned int ptrhash25_lookup(
    ...
    unsigned long long bucket = ptrhash_bucket_for(h1, num_buckets);
    unsigned char pilot = pilots[(unsigned int)bucket];
    unsigned long long slot = ptrhash_slot_for(h2, pilot, num_slots);
...
extern "C" __global__ void ani_ptrhash25_lookup_kernel(
    ...
    out_idx[tid] = ptrhash25_lookup(
        keys[tid], prehash_seed, mph_salt, num_buckets, num_slots, prerotate,
